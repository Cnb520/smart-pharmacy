...
import styles from './DrugAuthSub.module.css'

export default function DrugAuthSub() {
    ...
    const fetchDrugs = async () => {
      try {
        const { data } = await drugsApi.list({
          isPrescription: isPrescription ? 'true' : 'false',
          pageSize: 200,
        })
        setDrugs(data)
      } catch {
  ...
  const title = isPrescription ? '处方药' : '非处方药'
